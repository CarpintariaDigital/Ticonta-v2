...
def get_financial_report(
    ...
    service = ReportsService(db)
    return service.generate_financial_report(company_id=tenant_id, period=period)
...
def get_crm_report(
    ...
    service = ReportsService(db)
    return service.generate_crm_report(company_id=tenant_id, period=period)
...
def get_projects_report(
    ...
    service = ReportsService(db)
    return service.generate_project_report(company_id=tenant_id, period=period)
...
def get_hr_report(
    ...
    service = ReportsService(db)
    return service.generate_hr_report(company_id=tenant_id, period=period)
...
def export_report_csv(
    ...
    if report_type == ReportType.SALES:
        data = service.generate_sales_report(company_id=tenant_id).model_dump()
    elif report_type == ReportType.FINANCIAL:
        data = service.generate_financial_report(company_id=tenant_id, period=period).model_dump()
    elif report_type == ReportType.CRM:
        data = service.generate_crm_report(company_id=tenant_id, period=period).model_dump()
    elif report_type == ReportType.PROJECTS:
        data = service.generate_project_report(company_id=tenant_id, period=period).model_dump()
    elif report_type == ReportType.HR:
        data = service.generate_hr_report(company_id=tenant_id, period=period).model_dump()
    else:
