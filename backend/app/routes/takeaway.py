...
from app.dependencies.tenant import get_tenant_id, get_optional_tenant_id
...
def create_takeaway_order(
    ...
    return service.create_takeaway_order(data=data, company_id=tenant_id)
...
def get_takeaway_order(
    ...
    service = TakeawayService(db)
    return service.get_order(order_id=id, company_id=tenant_id)
...
def track_order_by_id(
    ...
    service = TakeawayService(db)
    return service.track_order(order_id_or_code=str(id), company_id=tenant_id)


@router.get("/track/{tracking_code}", response_model=OrderTrackingResponse)
def track_order_by_code(
    tracking_code: str,
    tenant_id: Optional[int] = Depends(get_optional_tenant_id),
    db: Session = Depends(get_db),
):
    """Rastreio público via código de rastreamento (ex: TC-A1B2C3)."""
    service = TakeawayService(db)
    return service.track_order(order_id_or_code=tracking_code, company_id=tenant_id or 1)
...
def update_order_status(
    ...
    service = TakeawayService(db)
    return service.update_order_status(order_id=id, data=data, company_id=tenant_id)
...
def assign_delivery_person(
    ...
    service = TakeawayService(db)
    return service.assign_delivery(order_id=id, data=data, company_id=tenant_id)
...
def update_delivery_status(
    ...
    service = TakeawayService(db)
    return service.update_delivery_status(order_id=id, data=data, company_id=tenant_id)
...
def get_pending_deliveries(
    ...
    service = TakeawayService(db)
    return service.get_pending_deliveries(company_id=tenant_id)
...
def get_takeaway_stats(
    ...
    service = TakeawayService(db)
    return service.get_takeaway_stats(company_id=tenant_id)
