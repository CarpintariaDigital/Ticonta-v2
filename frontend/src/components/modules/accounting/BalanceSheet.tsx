    ...
    <div className="space-y-5 rounded-2xl border border-zinc-200 bg-white backdrop-blur p-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-zinc-200 pb-4">
        <div>
          ...
          <p className="text-xs text-zinc-500">Posição em: {data.as_of_date}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-4 space-y-2 font-mono text-xs">
            <h4 className="font-bold text-zinc-800 uppercase tracking-wider mb-2">Composição do Activo</h4>
            {data.assets_breakdown.map((a) => (
              <div key={a.code} className="flex justify-between border-b border-zinc-200/40 pb-1.5">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-4 space-y-2 font-mono text-xs">
            <h4 className="font-bold text-zinc-800 uppercase tracking-wider mb-2">Passivo (Obrigações)</h4>
            {data.liabilities_breakdown.map((l) => (
              <div key={l.code} className="flex justify-between border-b border-zinc-200/40 pb-1.5">
                <span className="text-zinc-700">
            ...
            <h4 className="font-bold text-zinc-800 uppercase tracking-wider mt-4 mb-2 pt-2 border-t border-zinc-200">
              Capital Próprio & Resultados
            </h4>
            {data.equity_breakdown.map((eq) => (
              <div key={eq.code} className="flex justify-between border-b border-zinc-200/40 pb-1.5">
                <span className="text-zinc-700">
