...
export default function LeadDetails({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/40 backdrop-blur-sm p-4">
      <div className="flex h-[88vh] w-full max-w-3xl flex-col rounded-2xl border border-zinc-200 bg-zinc-50 shadow-2xl overflow-hidden">
        {/* Top Header */}
        <div className="flex items-center justify-between border-b border-zinc-200 bg-white px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              size="sm"
              onClick={onClose}
              className="h-8 w-8 p-0 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-full"
            >
              <X className="h-4 w-4" />
            </Button>
          </div>
        ...
        {/* Quick Actions Bar */}
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-200/80 bg-white px-6 py-2.5">
          <div className="flex items-center gap-1.5">
            <span className="text-xs font-semibold text-zinc-500 mr-1">Mover Estágio:</span>
            {(["novo", "proposta", "ganho", "perdido"] as LeadStage[]).map((st) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-xs font-semibold border transition-all ${
                  lead.stage === st
                    ? "bg-zinc-800 text-zinc-500 border-zinc-200 cursor-not-allowed"
                    : st === "ganho"
                    ...
                    : "bg-white text-zinc-700 border-zinc-200 hover:bg-zinc-800"
                }`}
              >
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-zinc-200 px-6 bg-white/40">
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {activeTab === "timeline" ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-4 space-y-2">
                <span className="text-xs font-bold uppercase tracking-wider text-zinc-500 block mb-2">
                  Dados de Contacto
                </span>
                <div className="flex justify-between border-b border-zinc-200/40 pb-1.5">
              ...
              <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-4 space-y-2">
                ...
                <div className="pt-2">
                  ...
                  <p className="text-zinc-700 bg-zinc-50 p-2 rounded-lg border border-zinc-200">
