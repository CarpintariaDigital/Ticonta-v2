...
export const OutstandingPaymentsModal: React.FC<OutstandingPaymentsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/40 backdrop-blur-sm animate-fade-in overflow-y-auto">
      <div className="bg-white border border-zinc-200/80 rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col overflow-hidden my-auto max-h-[85vh]">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-zinc-200 bg-zinc-50/80">
        ...
        <div className="p-4 bg-gradient-to-r from-amber-950/40 to-slate-900 border-b border-zinc-200 flex items-center justify-between">
          <div className="text-xs">
            <span className="text-zinc-500 block">Total a Receber ({items.length} registos)</span>
            <span className="text-xl font-black text-amber-400">
              {totalOutstanding.toLocaleString("pt-MZ")} MT
            </span>
          </div>

          <div className="flex gap-1 bg-white p-1 rounded-xl border border-zinc-200 text-xs">
            {[
              { id: "all", label: "Todos" },
              { id: "pos", label: "POS" },
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg font-semibold transition-all ${
                  moduleFilter === f.id ? "bg-amber-600 text-white" : "text-zinc-500 hover:text-white"
                }`}
              >
                {f.label}
              </button>
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-zinc-700">• {item.customer_name || "Cliente"}</span>
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase bg-slate-800 text-zinc-500">
                        {item.module_source}
                      </span>
                      {item.is_overdue && (
